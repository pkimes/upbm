...
Again, we start by identifying the set of sample GPRs in the directory, `gpr_dir`. In the quickstart example above, only raw `"Alexa"` GPR scans were captured (by default). However, there may also be Cy3 and Masliner GPR files in the same directory. To also capture these samples, we can change the `gpr_type=` parameter to any of `"Masliner"`, `"Cy3"`, or `"all"`.
...
```{r, fig.width = 15, fig.height = 5}
pbmPlotDensity(pbmdata, assay_name = "back",
...
```{r}
pbmdata_bc <- limmaBackgroundCorrect(pbmdata, assayb_name = "back")
```
