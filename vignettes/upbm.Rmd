...
The following code is only included for illustration and is not actually run. The data is read in as a single *PBMExperiment* object, with scans as columns and probes as rows. Scan information from `hoxc9table` is included in colData, and probe information is included in rowData. We split the *PBMExperiment* by scan type to create separate Alexa488 and Cy3 *PBMExperiment* objects for downstream analysis.
...
```{r}
hoxc9alexa450 <- hoxc9alexa[, colData(hoxc9alexa)$lp == 450]
```

We add unique IDs to the colData of the Alexa488 and Cy3 _PBMExperiment_ objects to uniquely match samples across the two objects. We can use information in the columns specifying the plate ID (`id`) and array position (`idx`) of each array, as this should be unique for each individual array. We combine these two columns into a single `uid` (unique ID) column.

```{r}
colData(hoxc9alexa450)$id <- gsub("(.*?)_.*", "\\1", colData(hoxc9alexa450)$id)
colData(hoxc9alexa450)$uid <- paste0(colData(hoxc9alexa450)$id, "_", colData(hoxc9alexa450)$idx)
colData(hoxc9cy3)$id <- gsub("(.*?)_.*", "\\1", colData(hoxc9cy3)$id)
colData(hoxc9cy3)$uid <- paste0(colData(hoxc9cy3)$id, "_", colData(hoxc9cy3)$idx)
```

First, we perform basic pre-processing on the probe intensities of each assay, including background subtraction, Cy3 normalization, and spatial adjustment. 

```{r}
hoxc9alexa450 <- backgroundSubtract(hoxc9alexa450, assay = "fore", assayb = "back")
```
...
After comparing the observed Cy3 intensities against the empirical reference, we normalize the Alexa488 scans according to the relative over or under abundance of dsDNA as measured by Cy3.

```{r}
hoxc9cy3e <- cy3FitEmpirical(hoxc9cy3, refcy3_8x60k_v1)
hoxc9alexa450 <- cy3Normalize(hoxc9alexa450, hoxc9cy3e, match_by = "uid")
```
...
```{r}
hoxc9alexa450 <- spatiallyAdjust(hoxc9alexa450)
```
...
```{r}
hoxc9alexa450 <- normalizeWithinReplicates(hoxc9alexa450)
hoxc9alexa450 <- normalizeAcrossReplicates(hoxc9alexa450)
```

Now that arrays have been properly pre-processed, we aggregate across replicates for each condition by fitting probe-level models to the data. These probe-level quantities are then used to obtain the final k-mer affinity estimates. By default, k-mer affinities are estimated for all unique 8-mers (including reverse complementation).

```{r}
hoxc9pfit <- probeFit(hoxc9alexa450)
hoxc9kfit <- kmerFit(hoxc9pfit)
```
