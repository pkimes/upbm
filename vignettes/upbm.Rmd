...
The returned object is a *PBMExperiment* with the same name number of columns as the original Alexa488 *PBMExperiment* object. The number of rows is decreased to only include probes designated as signal probes based on filtering criteria specified as part of the corresponding *PBMDesign*. In this case, this includes all probes with probe IDs prefixed by `dBr_`.
