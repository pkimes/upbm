...
In the quick start vignette, the pre-processing steps of background subtraction, Cy3 normalization, spatial adjustment, within and across replicate normalization were performed using the `upbmPreprocess(..)` wrapper function. Here, we cover the individual pre-processing steps and underlying functions in more detail. Additionally, we cover basic plotting functions which can be used to visualize the results of pre-processing at each step.
